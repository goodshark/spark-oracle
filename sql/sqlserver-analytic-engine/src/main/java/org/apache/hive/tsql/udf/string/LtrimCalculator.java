 ...
 */
public class LtrimCalculator extends BaseCalculator {
    public LtrimCalculator() {
    }

    @Override
    public Var compute() throws Exception {
        Var var = new Var("ltrim func", null, Var.DataType.NULL);
        if (getAllArguments().size() != 1)
            throw new FunctionArgumentException("ltrim", getAllArguments().size(), 1, 1);

        Var arg = getArguments(0);
        String argStr = StrUtils.trimQuot(arg.getVarValue().toString());
        String resStr = argStr.substring(argStr.indexOf(argStr.trim().charAt(0)));
