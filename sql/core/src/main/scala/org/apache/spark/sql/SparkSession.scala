...
class SparkSession private(
  ...
  private val sqlServerTable = new HashMap[Integer, HashMap[String, String]]()

  def getSqlServerTable: HashMap[Integer, HashMap[String, String]] = {
    sqlServerTable
  }
  ...
  def getRealTable(tableAliaName: String): String = {
    ...
    while (iterator.hasNext) {
      ...
      if (m.containsKey(tableAliaName)) {
        tbName = m.get(tableAliaName)
        return tbName
      }
    }
    tbName
  ...
  def getTables(t: Integer): HashSet[String] = {
    val tables: HashSet[String] = new HashSet[String]()
    if (sqlServerTable.containsKey(t)) {
      val m = sqlServerTable.get(t)
      val iterator = m.keySet().iterator()
      while (iterator.hasNext) {
        tables.add(m.get(iterator.next()))
      }
    }
    tables
  }
