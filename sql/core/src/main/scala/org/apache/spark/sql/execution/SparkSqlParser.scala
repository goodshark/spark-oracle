...
class SparkSqlAstBuilder(conf: SQLConf) extends AstBuilder {
  ...
  override def visitDelete(ctx: DeleteContext): LogicalPlan = withOrigin(ctx) {
    var table = visitTableIdentifier(ctx.deleteStatement().tableIdentifier())
    var tableNameAlias: String = null
    var tableNames: mutable.HashMap[String, TableIdentifier] = null
    if (null != ctx.deleteStatement().fromTable()) {
      tableNames = new mutable.HashMap[String, TableIdentifier]()
      val fromTable = ctx.deleteStatement().fromTable()
      tableNameAlias = table.table
      if (null != fromTable.strictIdentifier()) {
        tableNames += (fromTable.strictIdentifier().getText
          -> visitTableIdentifier(fromTable.tableIdentifier()))
      }
      ...
      if (tableNames.contains(tableNameAlias)) {
        table = tableNames.get(tableNameAlias).get
      }
    }
    return AcidDelCommand(ctx, table, tableNameAlias)
  }
  ...
  override def visitUpdate(ctx: UpdateContext): LogicalPlan = withOrigin(ctx) {
    ...
    var updateTable = visitTableIdentifier(ctx.updateStatement().tableIdentifier())
    var tableNameAlias: String = null
    var tableNames: mutable.HashMap[String, TableIdentifier] = null
  ...
  override def visitCreateIndex(ctx: CreateIndexContext): LogicalPlan = withOrigin(ctx) {
    val sql = ctx.start.getInputStream().getText(
      new Interval(ctx.start.getStartIndex(), ctx.stop.getStopIndex()))
    CreateIndexCommand(sql)
  }
